<template>
  <div v-if="store.showEdit && store.selected" class="fixed right-0 top-0 w-full max-w-md h-full bg-white shadow-lg p-6 z-50 overflow-auto">
    <h2 class="text-xl font-bold mb-4">Editar Asignación</h2>
    <form @submit.prevent="onSubmit">
      <label class="block mb-2">ID Solicitud
        <input v-model.number="store.selected.requestId" type="number" class="input input-bordered w-full" required min="1" />
      </label>
      <label class="block mb-2">ID Guía
        <input v-model.number="store.selected.guideId" type="number" class="input input-bordered w-full" required min="1" />
      </label>
